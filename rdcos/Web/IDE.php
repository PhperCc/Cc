<?php
//a:9:{s:4:"lang";s:2:"zh";s:9:"auth_pass";s:32:"d41d8cd98f00b204e9800998ecf8427e";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";i:1;s:7:"fm_root";s:23:"/home/system/rdcos/Apps";s:17:"cookie_cache_time";i:2592000;s:7:"version";s:15:"0.9.8 汉化版";}
//
// +--------------------------------------------------
// | Header and Globals
// +--------------------------------------------------	
...
function uppercase($str){
	global $charset;
    return strtoupper($str, $charset); //mb_
}
function lowercase($str){
	global $charset;
    return strtolower($str, $charset); //mb_
}
// +--------------------------------------------------
// | Interface
// +--------------------------------------------------
...
function show_tree(){
    ...
    echo "
        <tr>
        <td height=10 colspan=2><input type=button value=\"".et('Leave')."\" onclick='top.location.href=\"/\"'>
        </tr>
    ";
    echo "</table>\n";
    echo "</body>\n</html>";
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($current_dir)) {
        ...
        function view(arg){
            var w = 800;
            var h = 600;
            window.open('".addslashes($path_info["basename"])."?action=4&current_dir=".addslashes($current_dir)."&filename='+escape(arg), '', 'width='+w+',height='+h+',fullscreen=no,scrollbars=yes,resizable=yes,status=yes,toolbar=no,menubar=no,location=yes');
        }
        function rename(arg){
            var nome = '';
        ...
        $out .= "
            ...
            <input type=button value=\"".et('Leave')."\" onclick='top.location.href=\"/\"'>
            </nobr>";
        $uplink = "";
        
        if (substr_count($current_dir,'/')>3  && substr($current_dir,-1) != '/' || substr_count($current_dir,'/')>4  && substr($current_dir,-1) == '/'){
        ...
        if($entry_count){
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                ...
                if ($dir_entry["type"]=="dir"){
                    ...
                    $file_out[$file_count][] = "
                        <tr ID=\"entry$ind\" class=\"entryUnselected\" onmouseover=\"selectEntry(this, 'over');\" onmousedown=\"selectEntry(this, 'click');\">
                        <td><nobr><a href=\"JavaScript:view('".addslashes($file)."')\">$file</a></nobr></td>";
                    ...
                    else $file_out[$file_count][] = "<td>&nbsp;555</td>";
                    if ( is_readable($current_dir.$file) && (strpos(".txt#.sys#.bat#.ini#.conf#.swf#.php#.php3#.asp#.html#.htm#.jpg#.gif#.png#.bmp#", $dir_entry["ext"]."#" ) !== false)) $file_out[$file_count][] = "
