...
function server_ip()
{
    exec("ifconfig eth0", $lines);
    foreach($lines as $line)
    {
        if(preg_match("/inet addr:((?:\d{1,3}\.){3}\d{1,3})/", $line, $matches) > 0)
        {
            return $matches[1];
        }
    }
    return "";
    //$ss = exec('ifconfig eth0 | sed -n \'s/^ *.*addr:\\([0-9.]\\{7,\\}\\) .*$/\\1/p\'',$arr);
    //$ret = $arr[0];
    //return $ret;
}
...
<!--设备相关参数-->
<table>
  <tr><th colspan="4">设备参数</th></tr>
  <tr>
    <td>设备IP地址</td>
    <td colspan="3"><?php echo @get_current_user();?> - <?php $_SERVER['SERVER_NAME'];?>(<?php if('/'==DIRECTORY_SEPARATOR){echo server_ip() /*$_SERVER['SERVER_ADDR']*/;}else{echo @gethostbyname($_SERVER['SERVER_NAME']);} ?>)&nbsp;&nbsp;你的IP地址是：<?php echo @$_SERVER['REMOTE_ADDR'];?></td>
  </tr>
  <tr>
    <td>设备系统标识</td>
...
<?if("show"==$sysReShow){?>
<table>
  <tr><th colspan="6">设备实时数据</th></tr>
  <tr>
    <td width="13%" >设备当前时间</td>
    <td width="37%" ><span id="stime"><?php echo $stime;?></span></td>
    <td width="13%" >设备已运行时间</td>
...
<table>
  ...
  <tr>
    ...
    <td width="18%">
        <a href='<?php echo $phpSelf;?>?act=phpinfo' target='_blank'>PHPINFO</a>
    </td>
...
<!--设备性能检测-->
<table>
  <tr><th colspan="5">设备硬件性能检测</th></tr>
  <tr align="center">
    <td width="19%">参照对象</td>
...
<!--网络速度测试-->
<table>
	<tr><th colspan="3">到设备连接速度测试</th></tr>
  <tr>
...
<table>
  ...
  ?>
	...
	<table>
		<tr>
			<td class="w_foot"><a HREF="http://www.enh-tech.com" target="_blank"><?php echo "$title $version";?></a></td>
